...
RestaurantCategory.belongsTo(Category, {
...
RestaurantFavourite.belongsTo(Restaurant, {
  foreignKey: 'restaurantId'
});
RestaurantFavourite.belongsTo(User, {
  foreignKey: 'userId'
});

RestaurantRating.belongsTo(Restaurant, {
  foreignKey: 'restaurantId'
});
RestaurantRating.belongsTo(User, {
  foreignKey: 'userId'
});
...
Item.hasMany(MenuItemAddon, {
  foreignKey: 'menuItemId'
});
MenuItemAddon.belongsTo(AddonCategories, {
  foreignKey: 'categoryId'
});
...
module.exports = {
  ...
  allMerchants: async (req, res, requestData) => {
    ...
    const getRestaurants = await Restaurant.findAll({
      ...
      order: [
        [sequelize.literal('`distance`*1'), 'ASC']
      ],
      // ...(
  ...
  getRestaurantListByCatId: async (req, res) => {
    try {
      ...
      const restaurantsCountOpen = await RestaurantCategory.count({
        where: {
      ...
      const restaurantsCountClosed = await RestaurantCategory.count({
        where: {
          categoryId: requestData.categoryId,
        },
        include: [{
      ...
      let getRestaurantsClosed = [];
      const getRestaurantsOpen = await module.exports.getRestaurantsFromRestaurantCategories(req, res, requestData, 1);

      if (getRestaurantsOpen.length < parseInt(requestData.limit)) {

        // total 40  open 29  closed 11
        // limit 5
        ...
        if (getRestaurantsOpen.length != 0) {
          requestData.offset = 0;
        } else {
          requestData.offset = ((requestData.page - 1) * requestData.limit) - restaurantsCountOpen;
        }


        // console.log(Math.ceil(restaurantsCountOpen/parseInt(requestData.limit)), '=================Math.ceil(restaurantsCountOpen/parseInt(requestData.limit)');
        requestData.limit = parseInt(requestData.limit) - getRestaurantsOpen.length;

        console.log(requestData.page, '============>requestdata.page');
        console.log(requestData.offset, '============>requestdata.offset');

        getRestaurantsClosed = await module.exports.getRestaurantsFromRestaurantCategories(req, res, requestData, 0);
      }



      // console.log(getRestaurantsOpen, '====================getRestaurantsOpen'); 
      // console.log(getRestaurantsClosed, '====================getRestaurantsClosed'); 
      // console.log(getRestaurants); 
      // return; 
      // const restaurants = getRestaurants.map(data => data.toJSON());
      // console.log(JSON.stringify(restaurants, null, 2))

      // const restaurantimages = await models.restaurantImages.findAll({
      //   where: {
      //     restaurantId: 186
      //   },
      //   raw: true
      ...
      // const openRestaurants = results.filter(restaurantCategory => restaurantCategory.restaurant.status == 1).sort((a, b) => a.distance - b.distance);
      // const closedRestaurants = results.filter(restaurantCategory => restaurantCategory.restaurant.status == 0).sort((a, b) => a.distance - b.distance);
      // console.log(closedRestaurants, '=====>closedRestaurants');
      ...
      res.status(200).json({
        ...
        'message': 'Restaurants Listing.',
        'body': {
          // totalRestaurants: String(getRestaurantsOpen.count + getRestaurantsClosed.count),
          totalRestaurants: String(restaurantsCountOpen + restaurantsCountClosed),
          category,
          countTotal: getRestaurantsOpen.length + getRestaurantsClosed.length,
          countOpen: getRestaurantsOpen.length,
          countClosed: getRestaurantsClosed.length,
          restaurants: [
            ...getRestaurantsOpen,
            ...getRestaurantsClosed,
          ],
        },
      });
  ...
  getRestaurantListByCuisineId: async (req, res) => {
    try {
      ...
      const totalRestaurants = await RestaurantCuisine.findAll({
        where: {
      ...
      // getRestaurantsFromRestaurantCuisines


      let getRestaurantsClosed = [];
      const getRestaurantsOpen = await module.exports.getRestaurantsFromRestaurantCuisines(req, res, requestData, 1);


      if (getRestaurantsOpen.length < parseInt(requestData.limit)) {
        const restaurantsCountOpen = totalRestaurants.filter(value => value.restaurant.status == 1).length;
        console.log(getRestaurantsOpen.length, '======>getRestaurantsOpen');
        console.log(restaurantsCountOpen, '======>restaurantsCountOpen');
        ...
        getRestaurantsClosed = await module.exports.getRestaurantsFromRestaurantCuisines(req, res, requestData, 0);
      }
  ...
  getRestaurantDetails: async (req, res) => {
    try {
      ...
      let tempobj = {}
      tempobj.status = 1
     // tempobj.createdAt = sequelize.literal(('((`menus`.`open_time` <= DATE_ADD(curtime(),interval -4 hour) and `menus`.`close_time` >= DATE_ADD(curtime(),interval -4 hour)) or (`menus`.`open_time` ="" and `menus`.`close_time` = "")) and ((`menus`.days like CONCAT(CONCAT("%",DAYOFWEEK(DATE_ADD(now(),interval -4 hour))-1),"%")) or (`menus`.days=""))'))
        ...
        getRestaurant.menus = getRestaurant.menus.map(menus => {
          if (menus.items != null) {
            menus.items = menus.items.map(items => {
              ...
              items.restaurantName = getRestaurant.name;
    ...
    try {
      ...
      const favouriteRestaurantsListing = await RestaurantFavourite.findAll({
        ...
        include: [{
          ...
          model: models.restaurants,
          attributes: [
            'id', 'name', 'description', 'email', 'phone', 'image', 'address', 'city', 'province', 'country', 'latitude', 'longitude', 'status', 'minDelivery', 'deliveryStatus', 'deliveryType', 'isAvailable',
          ],
          include: [{
            model: models.restaurantImages,
          }]
      ...
      // console.log(favouriteRestaurantsListing); return false;
      ...
      if (favouriteRestaurantsListing.length > 0) {
        responseData = favouriteRestaurantsListing.map(async value => {
          ...
          value.restaurant.ratingsCount = rating[0].dataValues.ratings_count;
          value.restaurant.cuisines = cuisines;
  ...
  addRestaurantRating: async (req, res) => {
    try {
      ...
      if (req.files && req.files.image) {
        ...
        let upOrder = await addRestaurantRating.update({
          image: image
        }, {
          returning: true,
          where: {
      ...
      const addedRestaurantRating = await RestaurantRating.findOne({
        ...
        attributes: {
          include: [
            ...
            [sequelize.literal(`(SELECT AVG(rr.rating) FROM restaurant_ratings as rr WHERE rr.restaurant_id=restaurantRatings.restaurant_id)`), 'avgRating'],
            [sequelize.literal(`(SELECT count(rr.id) FROM restaurant_ratings as rr WHERE rr.restaurant_id=restaurantRatings.restaurant_id)`), 'ratingsCount'],
            // [sequelize.literal(`(SELECT count(rr.id) FROM restaurant_ratings as rr)`), 'ratingsCount'],
            // [sequelize.literal(`(SELECT \`restaurantRatings\`.\`id\`)`), 'ratingsCount'],
          ]
        },
        include: [{
          model: models.users,
          required: true,
          attributes: [
            'id', 'role', 'firstName', 'lastName', 'fullName', 'email', 'countryCode', 'phone', 'countryCodePhone', 'username', 'photo'
          ]
        }]
      });
      ...
      addedRestaurantRating.user.image = req.protocol + '://' + req.get('host') + '/images/users/' + addedRestaurantRating.user.image;
  ...
  getRestaurantsFromRestaurantCategories: async (req, res, requestData, status) => {
    console.log(status, '==========status');

    let restaurants = await RestaurantCategory.findAll({
      where: {
        categoryId: requestData.categoryId,
        // [Op.and]: [
        //   sequelize.literal(`\`restaurant\`.\`status\`=${status} && \`restaurant\`.\`hide_status\`=1`)
        // ]
      },
      // having: {
      // distance: {
      ...
      // },
      attributes: [
        // [sequelize.literal('`restaurant`.`id`'), 'id'],
        // 'restaurant.*',
        [
          sequelize.literal(
            "(SELECT (3959 *acos(cos( radians(" + requestData.latitude + ") ) * cos(radians( restaurant.latitude ) ) * cos(radians( restaurant.longitude ) - radians( " + requestData.longitude + " )) + sin(radians( " + requestData.latitude + " )) * sin(radians(restaurant.latitude)))) )"),
      ...
      include: [{
        ...
        attributes: ['id', 'name', 'description', 'email', 'image', 'latitude', 'longitude', 'address', 'minDelivery', 'deliveryType', 'isAvailable', 'status', 'createdAt', 'updatedAt',
          // [sequelize.literal(' (3959 * acos ( '
          //       + 'cos( radians('+requestData.latitude+') ) '
          //       + '* cos( radians( restaurant.latitude ) ) '
          //       + '* cos( radians( restaurant.longitude ) - radians('+requestData.longitude+') )' 
      ...
      ...(
        requestData.limit && requestData.page && status == 1 ?
        {
    ...
    if (restaurants.length > 0) {
      restaurants = await Promise.all(restaurants.map(async value => {
        value = value.toJSON();
        // console.log(value);
        const rating = await db.query(`SELECT AVG(rating) as avg_rating, count(id) as ratings_count  FROM restaurant_ratings as rr WHERE rr.restaurant_id=?`, {
          replacements: [value.restaurant.id],
  ...
  getRestaurantsFromRestaurantCuisines: async (req, res, requestData, status) => {
    console.log(status, '==========status');

    let restaurants = await RestaurantCuisine.findAll({
      where: {
        cuisineId: requestData.cuisineId,
      },
      having: {
      ...
      include: [{
        ...
        attributes: [
          ...
          'id', 'name', 'description', 'email', 'image', 'latitude', 'longitude', 'address', 'minDelivery', 'deliveryType', 'isAvailable', 'status', 'createdAt', 'updatedAt',

          // 'id', 'name', 'description', 'email', 'image', 'latitude', 'longitude', 'status', 'createdAt', 'updatedAt',
