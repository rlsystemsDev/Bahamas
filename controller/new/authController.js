...
//console.log(requestdata); return false;
            ...
            requestdata.countryCodePhone = requestdata.countryCode + requestdata.phone;
            // console.log(requestdata.countryCodePhone); return false;
            ...
            countryCodePhoneExists = countryCodePhoneExists.toJSON();
            // console.log(countryCodePhoneExists); return;
        ...
        try {
            ...
            if (requestdata != '') {
                ...
                let user_exists = await users.findOne({
                    ...
                    attributes: ['id', 'fullName', 'email', 'username', 'countryCode', 'phone', 'countryCodePhone', 'deviceType', 'deviceToken', 'created_at', 'updated_at']
                });
                ...
                found_user = await users.findOne({
                    where: { countryCodePhone: requestdata.phone },
                    attributes: ['id', 'fullName', 'email', 'username', 'country_code', 'phone', 'country_code_phone', 'loginType', 'photo', 'socialId', 'socialType', 'deviceType', 'deviceToken', 'createdAt', 'updatedAt']
                });
                    ...
                    .then(async phone_number => {
                        ...
                        if (phone_number.carrier.type == "voip") {
                            ...
                            const otpResponse = await client.messages.create(
                                {
                                    to: `+${requestData.countryCodePhone}`,
                                    from: '+13213961165',
                                    // body: `One time password is: ${requestData.otp} for Bahamas Eats`,
                                    body: `Your One Time Password is ${requestData.otp} for Bahama Eats. Please do not share this OTP with anyone.Thank you!`,
                                }
                            );
                            ...
                            user = {
                                ...
                                addressLatitude: address ? address.latitude : '',
                                addressLongitude: address ? address.longitude : '',
                            }
    ...
    socialLogin: async (req, res) => {
        try {
            ...
            if (requestData.profileImage) {
                requestData['photo'] = helper.fileUpload(requestData.photo, 'users');
            }

            let getUser = await helperFunctions.save(models['users'], requestData, true, req);
            ...
            return helperFunctions.success(res, 'User logged in successfully.', getUser);
        } catch (err) {
            return helperFunctions.error(res, err);
        }
    },
    ...
    //                 },
    //                 // attributes: ['id', 'email', 'password', 'username', 'fullName', 'countryCode', 'phone', 'photo', 'countryCodePhone', 'socialId', 'socialType', 'deviceType', 'deviceToken', 'createdAt', 'updatedAt']
    //             });
    //             if (!getUser) {
    //                 res.status(200).json({
    //                     'status': false,
    ...
    //                 where: { id: getUser.id },
    //                 // attributes: ['id', 'email', 'password', 'fullName', 'username', 'countryCode', 'phone', 'photo', 'countryCodePhone', 'socialId', 'socialType', 'deviceType', 'deviceToken', 'createdAt', 'updatedAt']
    //             });
        ...
        try {
            ...
            if (requestdata != '') {
                ...
                let getUser = await users.findOne({
                    ...
                    attributes: [
                        'id', 'email', 'username', 'fullName', 'createdAt', 'updatedAt'
                    ]
                });
                ...
                let forgot_password_html = 'Hello ' + getUser.username + ',<br> Your Forgot Password Link is: <a href="' + forgotPasswordUrl + '"><u>CLICK HERE TO RESET PASSWORD </u></a>. <br><br><br> Regards,<br> BahamaEats';
    ...
    verifyAccount: async (req, res) => {
        try {
            ...
            const isAuth = await users.findOne({
                where: {
                    forgotPasswordHash: requestData.hash,
                },
                raw: true,
            });

            if (!isAuth) {
                return res.render('accountVerificationMessage', {
            ...
            await users.update(
                ...
                {
                    returning: true, where: { id: isAuth.id }
                }
            );
